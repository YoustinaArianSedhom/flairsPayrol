...
export class ITHelpDesk implements CanActivate, CanLoad {
  ...
  canActivate(
    ...
    if (this._auth.isITHelpDesk) return true;
  ...
  isAuthorize(): Observable<boolean> {
    return this._store.select(AuthorizationState.isITHelpDesk).pipe(
      map( isITHelpDesk=> {
        if (isITHelpDesk) return true;
        return false;
    }));
  }
}
