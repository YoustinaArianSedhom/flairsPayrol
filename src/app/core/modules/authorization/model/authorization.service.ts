...
export class AuthorizationService {
  ...
  public setUserPermissions(permissions: PermissionModel[], roles: string[] = []): PermissionsListModel {
    const loggedInUserPermissions: PermissionsListModel = this._initialPermissions;
  ...
  public loadSystemRoles(): Observable<SystemRolesModel> {
    return this._http.fetch('Auth/GetSystemRoles').pipe(
      map((res : ApiResponse<SystemRolesModel>) => res.result)
    )
  }
