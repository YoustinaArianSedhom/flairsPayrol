      ...
      setTimeout(() => {
        ...
        return this.oauthService.refreshToken()
          .then(res => {
            ...
            if (window.location.href == window.location.origin + '/') window.location.href = window.location.origin + OrgConfigInst.ROUTES_CONFIG.root;
          ...
          .catch(e => {
            ...
            patchState({ tokenRefreshInProgress: false, lastDispatchedActions: [] })
            dispatch(new Navigate([OrgConfigInst.ROUTES_CONFIG.login]));
            dispatch(new ActivateSpinner)
          });
      }, 2800);
    }
